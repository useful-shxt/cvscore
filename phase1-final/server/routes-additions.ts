// ═════════════════════════════════════════════════════════════════════════════
// PHASE 1 ROUTE ADDITIONS
//
// HOW TO ADD:
// 1. At the top of server/routes.ts, add this import line after the existing imports:
//    import { callClaude, callClaudeHaiku, callClaudeSonnet, fetchPageText } from "./claude";
//
// 2. Paste EVERYTHING below the ─── START ─── line
//    BEFORE the closing } of registerRoutes()
//
// 3. Also apply the THREE FIXES noted below to existing routes
// ═════════════════════════════════════════════════════════════════════════════

// ─── EXISTING ROUTE FIX 1: callPerplexity — add maxTokens param ──────────────
// Find your existing callPerplexity function and change its signature to:
//
...
//     retries = 4,
//     useSearch = false,
//     maxTokens = 4096   ← ADD THIS PARAM
//   ): Promise<string> {
//     for (let attempt = 0; attempt <= retries; attempt++) {
//       const body: any = { model, messages, max_tokens: maxTokens };  ← USE IT HERE
//
// ─── EXISTING ROUTE FIX 2: Cover letters — replace entire route ──────────────
// Find: app.post("/api/cover-letters"
// Replace the ENTIRE route body with this (keep the app.post line):
//
//     try {
...
// ─── EXISTING ROUTE FIX 3: Interview prep — replace prompt inside /api/score/deep ──
// Find the prompt variable inside app.post("/api/score/deep"
// Replace it with this (keep everything else in the route):
//
...
// ─── START: PASTE EVERYTHING BELOW INTO routes.ts BEFORE CLOSING } ────────────
